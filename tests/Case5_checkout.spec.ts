...
test.describe('Case 5: Checkout Flow', () => {
    ...
    for (const data of checkoutData) {
        test(`Checkout succeeds with ${data.description}`, async ({ productPage, cartPage, checkoutPage }) => {
            const { customerName, customerPhone, customerAddress, paymentMethod } = data; // Get paymentMethod from the current data object
            ...
            await checkoutPage.verifySuccessScreen(customerName, customerAddress, paymentMethod); // Use the paymentMethod from the loop
